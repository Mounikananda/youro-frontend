...
const AdminPopUps = (props) => {
  ...
  const openPopup = () => {
    console.log("OPening popup :: ", props.data);
    setIsOpen(true);
  };
  ...
  const deletePrescription = async () => {
    ...
    const url =
      ...
      API_DETAILS.PORT +
      API_DETAILS.baseExtension +
      `/deletePrescription/${props.data.rowData["medicineId"]}`;
    const config = {
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "*",
  ...
  const deleteDiagnosis = async () => {
    ...
    const url =
      ...
      API_DETAILS.PORT +
      API_DETAILS.baseExtension +
      `/deleteDiagnosis/${props.data.rowData["diagId"]}`;
    const config = {
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "*",
  ...
  const deleteQuestionnaire = async () => {
    ...
    try {
      ...
      //console.error(err);
      toast.error(
        "There are underlying questions related to this questionnaire, unable to delete it until all the questions corresponding to it are deleted!!"
      );
    }
  };
  const deleteQuestion = async () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        toast.success("Deleted successfully!!");
        props.data.setParentRefreshStatus(true);
        props.data.postDeleteAction(res.data);
      })
      .catch((err) => {
        //console.error(err);
  ...
  const handleSubmit = () => {
    ...
    if (props.data.action === "delete-prescription") {
      ...
      setStep(temp);
      console.log(step);
    } else if (props.data.action === "delete-questionnaire") {
      console.log(props.data.rowData);
      deleteQuestionnaire();
      setIsOpen(false);
    } else if (props.data.action === "delete-question") {
      console.log(props.data.rowData);
      deleteQuestion();
      setIsOpen(false);
    }
  ...
  return (
    <div>
      ...
      {props.data.action == "delete-doctor" && step == 1 && (
        <>
          <Popup open={isOpen} closeOnDocumentClick onClose={closePopup} modal>
            <div className="popup-content-admin">
              <div className="row">
                <div className="col-12 info-col">
                  <h3 style={{ marginLeft: "5%" }}>
                    Are you sure you want to delete Dr.{" "}
                    {props.data.rowData.firstName} {props.data.rowData.lastName}
                    ?
                  </h3>
                </div>
      ...
      {props.data.action == "delete-questionnaire" && (
        <>
          <Popup open={isOpen} closeOnDocumentClick onClose={closePopup} modal>
            <div className="popup-content-admin">
              <div className="row">
      ...
      {props.data.action == "delete-question" && (
        <>
          <Popup open={isOpen} closeOnDocumentClick onClose={closePopup} modal>
            <div className="popup-content-admin">
              <div className="row">
