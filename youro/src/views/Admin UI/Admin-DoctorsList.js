...
import { Box, Typography, Container, Tooltip, Switch } from "@mui/material";
...
const AdminDoctorsList = () => {
  ...
  const handleApproveRenderAndChange = (
    cell = { emptyRow: true },
    isChange = false
  ) => {
    ...
    if (isChange) {
      for (let i = 0; i < tableData.length; i++) {
        ...
        if (cell.row.original.userId == tableData[i].userId) {
          ...
          tableData[i]["status"] =
            cell.row.original.status === "APPROVED" ? "PENDING" : "APPROVED";
              ...
              <MaterialReactTable
                ...
                renderDetailPanel={({ row }) => (
                  ...
                  >
                    <Container
                      maxWidth={false}
                      sx={{ padding: "0", margin: "0px", maxWidth: "100%" }}
                    >
                      <div className="row">
